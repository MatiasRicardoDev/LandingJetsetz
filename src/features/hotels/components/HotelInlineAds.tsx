...
import { getInlineHotelAds } from '../services/hotels.service';
...
export default memo(function HotelInlineAds({ maxItems, ...rest }: Props) {
  const { hotelSearchForm: params } = useHotelStore();

  const inlineAdsQuery = useQuery({
    queryKey: ['inlineHotelAds', params],
    queryFn: () => {
      const firstDate =
  ...
  const inlineItems = useMemo(() => {
    ...
    if (items.length < 4) {
      items.push({
        rank: 4,
        site: 'kayak.com',
        description: "Compare Hotels from 100's of travel sites.",
        headline: 'Kayak',
